...
|---|---|---|
| **snake_case** | `mod_id` en gradle.properties, assets/, packages Java | `better_connections` |
| **PascalCase** | Clases Java principales | `BetterConnections` |
| **camelCase** | Variables, métodos, config keys | `betterConnections` |
| **Title Case** | Display name en README, CHANGELOG, docs, CurseForge | `Better Connections` |

### Ficheros de documentación

| Fichero | Formato | Ejemplo |
|---|---|---|
| WORKFLOW | `WORKFLOW_<MOD_ID>_<MC-VERSION>.md` | `WORKFLOW_BETTER_CONNECTIONS_26-2.md` |
...
```
<mod_id>/                    # Repositorio Git único (contiene el .git/)
├── <minecraft_version>/     # Subcarpeta sin .git propio, existe en su rama
│   ├── build.gradle
│   ├── gradle.properties
│   ├── src/
│   ├── docs/
...
```
better_connections/          # Un solo repositorio Git (.git aquí)
...
```
better_connections/                        # Repositorio Git (.git aquí)
└── 26.2/                                  # Subcarpeta, existe en su propia rama
    ...
    ├── docs/
    │   ├── WORKFLOW_BETTER_CONNECTIONS_26-2.md   # Este documento
...
| `minecraft/26.2/neoforge-26.2.0.32-beta/production` | **Rama por defecto**. Rama de trabajo con todo el proyecto: código, docs/, lib_ext/, graphify-out/, tokens reales. Solo contiene los archivos de la versión 26.2 |
| `minecraft/26.1.2/neoforge-26.1.2.78/production` | Rama de trabajo de la versión 26.1.2. Solo contiene archivos de 26.1.2 |
| `minecraft/26.2/neoforge-26.2.0.32-beta/main` | Rama protegida. Recibe el mirror a GitHub. Solo contiene código fuente compilable. Se actualiza vía CI/CD con force push |
...
| Ejemplo | Significado |
|---|---|
| `better_connections-26.2-neoforge-0.0.0-beta.1.jar` | NeoForge 26.2, beta 1 |
| `better_connections-26.2-neoforge-1.0.0.jar` | NeoForge 26.2, release 1.0.0 |
...
```bash
# Beta
git tag -a 26.2-neoforge-beta.1 -m "v0.0.0-beta.1: Initial port from BetterWalls"
...
#    cp build/libs/better_connections-26.2-neoforge-0.0.0-beta.1.jar /ruta/a/la/instancia/mods/
#    rm /ruta/a/la/instancia/mods/better_connections-26.2-neoforge-version-anterior.jar
```
...
#    El JAR está en build/libs/better_connections-26.2-neoforge-<version>.jar
...
- **Sin residuos de mod original**: si el mod está basado en otro mod existente (fork/referencia), no debe quedar ningún rastro accidental del mod original. Revisar:
  - Nombres de paquetes (`com/lemonnik/betterwalls` → `com/skd/betterconnections`)
...
|---|---|---|
| 1.4.0 | 2026-07-27 | Versión inicial para 26.2: basado en WORKFLOW_BETTER_CONNECTIONS_26-1-2.md v1.4.0 |
